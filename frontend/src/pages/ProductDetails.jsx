...
const ProductDetails = () => {
  ...
  const {SetAllCustomerNameList} = useContext(CustomerContext)
  ...
  const handleSaleProduct = async() =>{
    try {
      ...
      const formData = {name:product.name,quantity:quantityToSale, customer:selectedCustomer }

      const response = await fetch('http://localhost:3000/api/product/sellproduct',{
        method:"POST",
        headers:{
  ...
  return (
    <div>
      ...
      <div className="mx-2 my-2 md:flex gap-5 justify-center">
        <div className=" w-full flex item-center justify-center my-0 md:w-3/5 lg:w-2/5 border-2 border-pink-500 rounded-xl ">
          <img
